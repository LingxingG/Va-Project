...
rain + geom_violin(fill="lightblue")+
  geom_boxplot(width=0.1, color="white", alpha=0.2)+  stat_summary(fun=mean, colour="darkred", geom="point",
                                                                   shape=18, size=3,show.legend = FALSE)+stat_summary(fun.data = fun_mean, geom="text", vjust=-0.9, size=3.5)+   ggtitle ("Rainfall distribution by month") +  xlab("Month") +  ylab ("Average Rainfall ( mm )")
```
...
temp + geom_violin(fill="lightblue")+
  ...
  xlab("Month") +  ylab ("Average temperature ( ºC )") +theme_minimal()
```
...
```{r}
ggplot(temperature, aes(x = mean_temp, y = Month, fill = stat(x))) +
